...
- **Unscoped Invocation Expansion:** Top-Level Unscoped Invocations must undergo their recursive Expansion **independently** of other Top-Level Sibling Unscoped Invocations. Their resulting Definitions cannot be naively injected into the Parent Context during a left-to-right walk, as this would alter the context for siblings. The Unscoped Invocations must either cache their definitions in a separate pool or be kept encapsulated until every sibling Unscoped Invocation has been explored, only being hoisted afterward.
...
- **Local Pre-Patterns on Children:** The Parent applies its own Local Pre-Patterns to the Payloads of its non-Invocation Child Nodes _before_ the Child Nodes are Executed. The Child cannot do this itself because it cannot distinguish between the Parent's Local Pre-Patterns and inherited Grandparent Pre-Patterns.
- **Pre-Patterns on Resolved Values:** The Resolved Value of Invoked Definitions appears completely Raw to the system. Therefore, the _entire stack_ of inherited and local Pre-Patterns should be applied to the Resolved Value before further processing. TODO review if this is still accurate with Eager Definitions.
- **Local Post-Patterns:** Applied bottom-up. A Scope applies _only_ its Local Post-Patterns to the concatenated Literal Text resulting from its execution _before_ returning that Literal string to the Parent. Any other Post-Patterns from the inherited Context will be applied when the associated higher-level Node finishes Execution.
...
- **Error: Conflating "Evaluation" with "Node Execution" and "Payload" with "Resolved Value".**
  - _Source of Error:_ Abstractly generalizing the engine's text processing instead of adhering strictly to the `GLOSSARY.md` definitions. Don't treat any string being processed as a "Payload," missing the crucial architectural distinction between parsing user input and processing a dictionary lookup result.
...
- **Error: Creating Scope as "Step 1" of the Phase 1 Pipeline.**
  - _Source of Error:_ Attempting to neatly package Scope Expansion and Token Parsing into a single top-to-bottom block. This fundamentally broke the isolation rule required for Sibling Unscoped Invocations, which must Expand in the exact same inherited Context before mutating anything.
- **Error: Attempting to have Child Nodes apply the Parent's Local Pre-Patterns.**
  - _Source of Error:_ Trying to solve the "Pre-Pattern Paradox" (where Local Pre-Patterns can't apply to their own block before Lexing) by delegating it to the child payload. A child has no built-in way to identify the "boundary" of the Parent's Local level within the inherited Context vs a Grandparent.
...
2. **Respect Glossary Capitalization:** If the user capitalizes a word (e.g., "Scope", "Token", "Evaluation"), you must treat it strictly as the defined entity from `GLOSSARY.md`. Do not use these words casually to refer to a meaning other than the one in `GLOSSARY.md`.
3. **Strictly Maintain Unscoped Independence:** When detailing Unscoped Invocation Expansion, ensure that Top-Level Unscoped Invocations are Expanded using the baseline Inherited and Top-Level Context. Do not let their hoisted Definitions leak into the Context used to expand their siblings.
4. **Tie Scope to Execution, Not Parsing:** Enforce Scope transitions (pushing/popping a Sentinel or Local Definitions) specifically around the step where a Child Node is Executed, unifying the handling of Text nodes, Scope nodes, and Invocations.
5. **Enforce the Applied-Once Principle:** Route Pre-Patterns carefully. Ensure you clearly state that a Child Node's Payload is mutated with the Parent's Local Pre-Patterns, while _Resolved Values_ from Dictionary lookups receive the entire Context Stack's Pre-Patterns.
