...
TODO: Make this dynamic based on the characters passed to the lexer
...
**Opening Boundary:** eg `<` or `{` (unescaped)
- Starts a BOUNDED_OPEN token
- Must not be preceded by escape character (eg `\`)

**Closing Boundary:** eg `>` or `}` (unescaped)
- Starts a BOUNDED_CLOSE token
- Must not be preceded by escape character (eg `\`)
- Matches innermost unmatched BOUNDED_OPEN of the same type
## Nesting rules
### Examples
