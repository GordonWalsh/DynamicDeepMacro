...
- **3.3: Payload:** Raw unprocessed string content of an object, stripped of its outer syntax boundaries (e.g., the text inside `< >` for an Invocation, the literal string of a TextNode, the raw Value-Pattern of a Definition, etc.).
- **3.4: Resolved Value:** Final string returned after the engine processes a Key-String against the Context (including concatenation of Definitions, array indexing, and TBD Regex substitutions), but before beginning to Evaluate it as Raw Text. TODO: Are there situations where this would be considered a Literal Text, not Raw; and what's the practical meaning re Regex-Regex Definitions and direct Regex substitutions?
...
- **Footnote Architecture:** The requirement that definitions are order-independent within a block. A definition at the bottom of a payload correctly applies to an invocation at the top. within an Invocation or Group (e.g., the text inside `< >` or `{ }`).
