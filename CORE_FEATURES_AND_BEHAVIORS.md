...
**7. Unscoped Invocations (Scope Injection)** Processing an Invocation with a leading Split marker (`<|container|:key:val>`) does _not_ isolate the contents and provides its resulting Nodes and contained Definitions to the parent Scope. This allows macros to act as Definition Containers whose rules seamlessly apply to their siblings.
