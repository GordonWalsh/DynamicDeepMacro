...
The Lexer converts raw strings into a flat list of `Token` objects. It simply divides the input strings into chunks without providing any interpretation or processing.
...
To solve the paradox of malformed brackets (e.g., `< { > }`) and other potentially-overlapping syntax, the Lexer employs an interval culling algorithm.
TODO: If a syntax character is consumed by an overriding interval, will the surrounding characters of the same type "look past" that character to match as though it wasn't there, or does that break the whole interval candidate that could have been part of?
...
At this point, we have fully "entered" the Node, and (I think) should be reliant on just the Context object for any engine state information needed. The Payload string is now chunked into a List of Tokens (though not 100% on whether that List is flat or pre-Segmented at the detected Split Tokens). The processing pipeline branches into two paths based on whether the Payload is Invocation-like or Raw Text content.
...
1. Separate the token List at Top-Level `SPLIT` Tokens to get a List of Sublists, each representing one Segment with Modifier still attached.
    - TODO: It looks like this step may become part of the Lexer with an "invocation behavior" flag that changes the iteraction between Definitions and Split markers. The Lexing may stay as part of a separate "Phase 0" Pre-Processing or may be absorbed into the Phase 1 of the specific Node logics.
...
5. **Selection:** For each Resolved Value, (first Lex if needed, then) use the associated Modifier or default to single random Option to Apply PRNG Selection to get a flat Token List.
...
1. **Parsing:** Parse the Token List into a list of Child AST Nodes and Top-Level Definitions. TODO: Definition handling as emitted objects or by modifying a passed DefLibrary? Elaborated elsewhere.
...
2. **Positional Early-Capture (`PositionalNode` handling):**
