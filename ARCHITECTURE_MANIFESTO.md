...
### Evaluation & Determinism (The "Anti-Precarious" Directive)

**Axiom: The engine must never produce "precarious" strings.**
A precarious string is one that behaves like dead data until it is unexpectedly touched by an unrelated process (like a distant Pre-Pattern or concatenation into a New Root), causing it to spontaneously detonate into active syntax.

To guarantee determinism:

1. **Evaluation is Continuous:** Evaluation is not a one-time toggle. It is the continuous reduction of an expression until no valid syntax remains.
2. **Armor is Permanent:** If a user wants syntax to remain completely inert regardless of how the text is mutated or passed around, they must use the Escape syntax. The engine will not artificially protect `LITERAL` text from re-evaluation if its structural geometry changes.

### Escaping Mechanics & The Final Cleaner

To support Terminal Armor without requiring nodes to perform expensive localized string replacements, all escape consumption is deferred to the absolute end of the execution pipeline.

#### The Final Orchestrator Cleaner

When the Orchestrator has reduced the entire AST to a final flat list of `TextNodes` and executes `"".join()`, it passes the resulting monolithic string through a single, highly optimized cleaner function.

This cleaner performs two operations:

1. **Block Escapes:** Strips all valid `</` and `/>` pairs.
2. **Inline Escapes:** Strips the backslash `\` **strictly and only** if it precedes an engine-reserved character (e.g., `\<`, `\>`, `\{`, `\|`, `\:`, `\\`).

#### Inline Escapes and Resolution Parity

Because inline escapes (`\`) survive AST evaluation, they provide perfect parity during dictionary resolution.

- The Lexer ignores `\:` during its structural sweep.
- A Definition's Key-Pattern evaluates to `"User\:Input"`.
- An Invocation's Key evaluates to `"User\:Input"`.
- The Resolver successfully matches them using a standard `==` string comparison. No localized `.replace()` logic is required in the AST nodes.

#### The Double-Escape Handshake (Regex Compatibility)

Because the Final Cleaner only strips `\` from macro-reserved characters, standard Python regex escapes (`\d`, `\n`, `\w`) pass through the engine completely unharmed.

_Trap / Guideline:_ If a user is writing a Regex Value and needs to use a regex syntax element that _includes_ a macro-reserved character—such as a capture group `\g<1>`—the macro Lexer will misinterpret `<1>` as an Invocation. The user must explicitly double-escape the macro syntax: `\g\<1\>`. The Final Cleaner will strip the `\`, successfully handing `\g<1>` to the underlying Python Regex engine.

### Post-Patterns & Syntax Injection

The execution of syntax dynamically generated by Post-Patterns depends entirely on the hierarchical depth at which the Post-Pattern is applied.

- **Deep Injection:** If a Post-Pattern injects engine syntax (e.g., `<Macro>`) inside a deeply nested Node, that Node's parent scope will still consider the returned text "live" (not in Normal Form). The Lexer will sweep the New Root, detect the syntax, and evaluate it natively.
- **Root Injection:** If a Post-Pattern is applied at the absolute, outermost root level of the user's input, the evaluation loop has already halted. The injected `<Macro>` will be output as raw, unparsed text.
- _Guideline:_ The engine takes no responsibility for evaluating syntax injected after the orchestrator has finalized the root AST. If users wish to inject top-level syntax via Post-Patterns and have it evaluate, they must architect their own two-pass parser loop outside of the core engine.
